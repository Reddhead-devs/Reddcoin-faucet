...
{	
	...
	* @desc Gets info about a specific transaction
	*
	* @param string $hash
	*
	* @return object
	*/
    public function gettransaction($hash)
    {
        if($this->connection)
		{
			try
			{
				$return = $this->connection->getblock($hash);
			}
			catch(Exception $e)
			{
				$return = '';
    ...
    /**
	* @method getaccount
	* @desc Gets the signature of provided address
	*
	* @param string $hash
	*
	* @return object
	*/
    public function getaccount($address)
    {
        if($this->connection)
		{
			try
			{
				$return = $this->connection->getaccount($address);
			}
			catch(Exception $e)
			{
				$return = '';
	...
	*/
    ...
    /**	
	* @method move
	* @desc Query server and move coins from one account to other
	*
	...
	* @return string
	*/
	public function move($fromLabel = '', $toLabel = '', $amount_in_satoshi = 0, $minimum_confirmations = 1)
	{
		$tx_id = false;
		
		$amount_in_satoshi = $amount_in_satoshi;
		...
		if($amount_in_coins >= 21000000.0)
		{
			...
			if($this->connection){
				...
				try
				{
					$tmp_verifymessage_status = $this->connection->move($fromLabel, $toLabel, $amount_in_coins, $minimum_confirmations, '');
					$tmp_command_success = 1;
				}
				catch(Exception $e)
				{
